...
exports.createRespuesta = async (req, res) => {
  try {
    ...
    const pregunta = await require('../models/pregunta.model').findById(idPregunta);
    if (!pregunta) return res.status(400).json({ message: 'Pregunta no encontrada' });
    ...
    if (esLaCorrecta) {
      const yaCorrecta = await Respuesta.findOne({ idPregunta, esLaCorrecta: true });
      if (yaCorrecta) {
        return res.status(400).json({ message: 'Ya existe una respuesta correcta para esta pregunta.' });
      }
    }
...
exports.updateRespuesta = async (req, res) => {
  try {
    ...
    if (typeof esLaCorrecta !== 'undefined') {
      if (esLaCorrecta) {
        const yaCorrecta = await Respuesta.findOne({ idPregunta: respuesta.idPregunta, esLaCorrecta: true, _id: { $ne: respuesta._id } });
        if (yaCorrecta) {
          return res.status(400).json({ message: 'Ya existe otra respuesta correcta para esta pregunta.' });
        }
      }
      respuesta.esLaCorrecta = esLaCorrecta;
    }
    if (req.file) {
      respuesta.fotoUrl = await uploadImageToCloudinary(req.file.path, 'respuestas');
      fs.unlinkSync(req.file.path);
